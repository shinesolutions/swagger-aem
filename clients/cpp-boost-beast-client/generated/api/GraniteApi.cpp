...
GraniteApiException::GraniteApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status GraniteApiException::getStatus() const
{
    return m_status;
}
...
GraniteApi::sslSetup(
    ...
    std::string path = m_context + "/libs/granite/security/post/sslSetup.html";
    ...
    formParameters.emplace_back(
        "privatekeyFile",
        toFormParameterValue(privatekeyFile),
        true);
    formParameters.emplace_back(
        "certificateFile",
        toFormParameterValue(certificateFile),
        true);
    ...
    const char* queryParameterSeparator = "?";
        ...
        appendQueryParameter(
            ...
            serializeQueryParameterValue(keystorePasswordConfirm));
        appendQueryParameter(
            queryParameterStream,
            queryParameterSeparator,
            "truststorePassword",
            serializeQueryParameterValue(truststorePassword));
        appendQueryParameter(
            queryParameterStream,
            queryParameterSeparator,
            "truststorePasswordConfirm",
            serializeQueryParameterValue(truststorePasswordConfirm));
        appendQueryParameter(
            queryParameterStream,
            queryParameterSeparator,
            "httpsHostname",
            serializeQueryParameterValue(httpsHostname));
        appendQueryParameter(
            queryParameterStream,
            queryParameterSeparator,
            "httpsPort",
            serializeQueryParameterValue(httpsPort));
...
std::string GraniteApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void GraniteApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void GraniteApi::handleStdException(const std::exception&) {
    throw;
}

void GraniteApi::handleUncaughtException() {
    throw;
}
