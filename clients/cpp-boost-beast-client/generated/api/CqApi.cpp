...
CqApiException::CqApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status CqApiException::getStatus() const
{
    return m_status;
}
...
CqApi::getLoginPage(
    ...
    std::string path = m_context + "/libs/granite/core/content/login.html";
...
CqApi::postCqActions(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/.cqactions.html";
    std::map<std::string, std::string> headers;
    // query params
    std::stringstream queryParameterStream;
    const char* queryParameterSeparator = "?";
        ...
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "changelog",
            serializeQueryParameterValue(changelog));
...
std::string CqApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void CqApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void CqApi::handleStdException(const std::exception&) {
    throw;
}

void CqApi::handleUncaughtException() {
    throw;
}
