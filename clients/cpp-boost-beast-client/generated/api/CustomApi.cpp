...
CustomApiException::CustomApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status CustomApiException::getStatus() const
{
    return m_status;
}
...
CustomApi::getAemHealthCheck(
    ...
    if (combineTagsOr) {
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "combineTagsOr",
            serializeQueryParameterValue(*combineTagsOr));
    }
...
void
CustomApi::postConfigAemHealthCheckServlet(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/apps/system/config/com.shinesolutions.healthcheck.hc.impl.ActiveBundleHealthCheck";
    std::map<std::string, std::string> headers;
    // query params
    std::stringstream queryParameterStream;
    const char* queryParameterSeparator = "?";
    ...
    if (bundlesPeriodignoredAtTypeHint) {
        appendQueryParameter(
            ...
            serializeQueryParameterValue(*bundlesPeriodignoredAtTypeHint));
    }
    path += queryParameterStream.str();
...
void
CustomApi::postConfigAemPasswordReset(
    ...
    std::string serializedRequestBody;
    std::string path = m_context + "/apps/system/config/com.shinesolutions.aem.passwordreset.Activator";
    std::map<std::string, std::string> headers;
    // query params
    std::stringstream queryParameterStream;
    const char* queryParameterSeparator = "?";
    ...
    if (pwdresetPeriodauthorizablesAtTypeHint) {
        appendQueryParameter(
            ...
            serializeQueryParameterValue(*pwdresetPeriodauthorizablesAtTypeHint));
    }
    path += queryParameterStream.str();
...
std::string CustomApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void CustomApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void CustomApi::handleStdException(const std::exception&) {
    throw;
}

void CustomApi::handleUncaughtException() {
    throw;
}
