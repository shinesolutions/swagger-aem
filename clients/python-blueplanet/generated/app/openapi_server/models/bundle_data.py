# coding: utf-8

from __future__ import absolute_import
from datetime import date, datetime  # noqa: F401

from typing import List, Dict  # noqa: F401

from app.openapi_server.models.base_model import Model
from app.openapi_server.models.bundle_data_prop import BundleDataProp  # noqa: F401,E501
from openapi_server import util


class BundleData(Model):
    """NOTE: This class is auto generated by the swagger code generator program.

    Do not edit the class manually.
    """

    def __init__(self, id: int=None, name: str=None, fragment: bool=None, state_raw: int=None, state: str=None, version: str=None, symbolic_name: str=None, category: str=None, props: List[BundleDataProp]=None):  # noqa: E501
        """BundleData - a model defined in Swagger

        :param id: The id of this BundleData.  # noqa: E501
        :type id: int
        :param name: The name of this BundleData.  # noqa: E501
        :type name: str
        :param fragment: The fragment of this BundleData.  # noqa: E501
        :type fragment: bool
        :param state_raw: The state_raw of this BundleData.  # noqa: E501
        :type state_raw: int
        :param state: The state of this BundleData.  # noqa: E501
        :type state: str
        :param version: The version of this BundleData.  # noqa: E501
        :type version: str
        :param symbolic_name: The symbolic_name of this BundleData.  # noqa: E501
        :type symbolic_name: str
        :param category: The category of this BundleData.  # noqa: E501
        :type category: str
        :param props: The props of this BundleData.  # noqa: E501
        :type props: List[BundleDataProp]
        """
        self.swagger_types = {
            'id': int,
            'name': str,
            'fragment': bool,
            'state_raw': int,
            'state': str,
            'version': str,
            'symbolic_name': str,
            'category': str,
            'props': List[BundleDataProp]
        }

        self.attribute_map = {
            'id': 'id',
            'name': 'name',
            'fragment': 'fragment',
            'state_raw': 'stateRaw',
            'state': 'state',
            'version': 'version',
            'symbolic_name': 'symbolicName',
            'category': 'category',
            'props': 'props'
        }

        self._id = id
        self._name = name
        self._fragment = fragment
        self._state_raw = state_raw
        self._state = state
        self._version = version
        self._symbolic_name = symbolic_name
        self._category = category
        self._props = props

    @classmethod
    def from_dict(cls, dikt) -> 'BundleData':
        """Returns the dict as a model

        :param dikt: A dict.
        :type: dict
        :return: The BundleData of this BundleData.  # noqa: E501
        :rtype: BundleData
        """
        return util.deserialize_model(dikt, cls)

    @property
    def id(self) -> int:
        """Gets the id of this BundleData.

        Bundle ID  # noqa: E501

        :return: The id of this BundleData.
        :rtype: int
        """
        return self._id

    @id.setter
    def id(self, id: int):
        """Sets the id of this BundleData.

        Bundle ID  # noqa: E501

        :param id: The id of this BundleData.
        :type id: int
        """

        self._id = id

    @property
    def name(self) -> str:
        """Gets the name of this BundleData.

        Bundle name  # noqa: E501

        :return: The name of this BundleData.
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name: str):
        """Sets the name of this BundleData.

        Bundle name  # noqa: E501

        :param name: The name of this BundleData.
        :type name: str
        """

        self._name = name

    @property
    def fragment(self) -> bool:
        """Gets the fragment of this BundleData.

        Is bundle a fragment  # noqa: E501

        :return: The fragment of this BundleData.
        :rtype: bool
        """
        return self._fragment

    @fragment.setter
    def fragment(self, fragment: bool):
        """Sets the fragment of this BundleData.

        Is bundle a fragment  # noqa: E501

        :param fragment: The fragment of this BundleData.
        :type fragment: bool
        """

        self._fragment = fragment

    @property
    def state_raw(self) -> int:
        """Gets the state_raw of this BundleData.

        Numeric raw bundle state value  # noqa: E501

        :return: The state_raw of this BundleData.
        :rtype: int
        """
        return self._state_raw

    @state_raw.setter
    def state_raw(self, state_raw: int):
        """Sets the state_raw of this BundleData.

        Numeric raw bundle state value  # noqa: E501

        :param state_raw: The state_raw of this BundleData.
        :type state_raw: int
        """

        self._state_raw = state_raw

    @property
    def state(self) -> str:
        """Gets the state of this BundleData.

        Bundle state value  # noqa: E501

        :return: The state of this BundleData.
        :rtype: str
        """
        return self._state

    @state.setter
    def state(self, state: str):
        """Sets the state of this BundleData.

        Bundle state value  # noqa: E501

        :param state: The state of this BundleData.
        :type state: str
        """

        self._state = state

    @property
    def version(self) -> str:
        """Gets the version of this BundleData.

        Bundle version  # noqa: E501

        :return: The version of this BundleData.
        :rtype: str
        """
        return self._version

    @version.setter
    def version(self, version: str):
        """Sets the version of this BundleData.

        Bundle version  # noqa: E501

        :param version: The version of this BundleData.
        :type version: str
        """

        self._version = version

    @property
    def symbolic_name(self) -> str:
        """Gets the symbolic_name of this BundleData.

        Bundle symbolic name  # noqa: E501

        :return: The symbolic_name of this BundleData.
        :rtype: str
        """
        return self._symbolic_name

    @symbolic_name.setter
    def symbolic_name(self, symbolic_name: str):
        """Sets the symbolic_name of this BundleData.

        Bundle symbolic name  # noqa: E501

        :param symbolic_name: The symbolic_name of this BundleData.
        :type symbolic_name: str
        """

        self._symbolic_name = symbolic_name

    @property
    def category(self) -> str:
        """Gets the category of this BundleData.

        Bundle category  # noqa: E501

        :return: The category of this BundleData.
        :rtype: str
        """
        return self._category

    @category.setter
    def category(self, category: str):
        """Sets the category of this BundleData.

        Bundle category  # noqa: E501

        :param category: The category of this BundleData.
        :type category: str
        """

        self._category = category

    @property
    def props(self) -> List[BundleDataProp]:
        """Gets the props of this BundleData.


        :return: The props of this BundleData.
        :rtype: List[BundleDataProp]
        """
        return self._props

    @props.setter
    def props(self, props: List[BundleDataProp]):
        """Sets the props of this BundleData.


        :param props: The props of this BundleData.
        :type props: List[BundleDataProp]
        """

        self._props = props
