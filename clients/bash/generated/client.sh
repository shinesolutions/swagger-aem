...
operation_parameters_minimum_occurrences["postAgent:::jcr:content/aliasUpdate"]=0
...
operation_parameters_minimum_occurrences["postTreeActivation:::cmd"]=1
...
operation_parameters_maximum_occurrences["postAgent:::jcr:content/aliasUpdate"]=0
...
operation_parameters_maximum_occurrences["postTreeActivation:::cmd"]=0
...
operation_parameters_collection_type["postAgent:::jcr:content/aliasUpdate"]=""
...
operation_parameters_collection_type["postTreeActivation:::cmd"]=""
...
${BOLD}${WHITE}Adobe Experience Manager (AEM) API command line client (API version 3.7.1-pre.0)${OFF}
...
print_about() {
    echo ""
    echo -e "${BOLD}${WHITE}Adobe Experience Manager (AEM) API command line client (API version 3.7.1-pre.0)${OFF}"
    echo ""
    echo -e "License: "
...
print_version() {
    echo ""
    echo -e "${BOLD}Adobe Experience Manager (AEM) API command line client (API version 3.7.1-pre.0)${OFF}"
    echo ""
}
...
print_postAgent_help() {
    ...
    echo -e "  * ${GREEN}jcr:content/cq:template${OFF} ${BLUE}[string]${OFF} ${CYAN}(default: null)${OFF} - ${YELLOW} Specify as: jcr:content/cq:template=value${OFF}" \
        | paste -sd' ' - | fold -sw 80 | sed '2,$s/^/    /'
    echo -e "  * ${GREEN}jcr:content/aliasUpdate${OFF} ${BLUE}[boolean]${OFF} ${CYAN}(default: null)${OFF} - ${YELLOW} Specify as: jcr:content/aliasUpdate=value${OFF}" \
        | paste -sd' ' - | fold -sw 80 | sed '2,$s/^/    /'
...
print_postTreeActivation_help() {
    ...
    echo -e "  * ${GREEN}path${OFF} ${BLUE}[string]${OFF} ${RED}(required)${OFF} ${CYAN}(default: null)${OFF} - ${YELLOW} Specify as: path=value${OFF}" \
        | paste -sd' ' - | fold -sw 80 | sed '2,$s/^/    /'
    echo -e "  * ${GREEN}cmd${OFF} ${BLUE}[string]${OFF} ${RED}(required)${OFF} ${CYAN}(default: activate)${OFF} - ${YELLOW} Specify as: cmd=value${OFF}" \
        | paste -sd' ' - | fold -sw 80 | sed '2,$s/^/    /'
    echo ""
    echo -e "${BOLD}${WHITE}Responses${OFF}"
    code=0
...
call_postTreeActivation() {
    ...
    # ignore error about 'query_parameter_names' being unused; passed by reference
    # shellcheck disable=SC2034
    local query_parameter_names=(ignoredeactivated onlymodified path cmd  )
    local path

    if ! path=$(build_request_path "/libs/replication/treeactivation.html" path_parameter_names query_parameter_names); then
        ERROR_MSG=$path
        exit 1
    fi
    local method="POST"
