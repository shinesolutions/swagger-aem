...
defmodule AdobeExperienceManager(AEM)API.Connection do
  @moduledoc """
  Handle Tesla connections for AdobeExperienceManager(AEM)API.

  Additional middleware can be set in the compile-time or runtime configuration:

      config :tesla, AdobeExperienceManager(AEM)API.Connection,
        base_url: "http://localhost",
        adapter: Tesla.Adapter.Hackney

  The default base URL can also be set as:

      config :"adobe_experience_manager(aem)api",
        :base_url, "http://localhost"
  """

  @default_base_url Application.compile_env(
                      :"adobe_experience_manager(aem)api",
                      :base_url,
                      "http://localhost"
                    )
  ...
  @doc """
  Configure a AdobeExperienceManager(AEM)API client.

  ### Parameters

  - `options`: an optional keyword list of AdobeExperienceManager(AEM)API.Connection.options.
  ...
  def middleware(options \\ []) do
    base_url =
      Keyword.get(
        options,
        :base_url,
        Application.get_env(:"adobe_experience_manager(aem)api", :base_url, @default_base_url)
      )
    ...
    user_agent =
      Keyword.get(
        ...
        Keyword.get(
          tesla_options,
          :user_agent,
          "openapi-generator - AdobeExperienceManager(AEM)API 1.0.0 - elixir"
        )
      )
