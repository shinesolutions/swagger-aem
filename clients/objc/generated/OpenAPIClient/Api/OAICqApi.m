...
@implementation OAICqApi

NSString* kOAICqApiErrorDomain = @"OAICqApiErrorDomain";
NSInteger kOAICqApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) getLoginPageWithCompletionHandler: 
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/libs/granite/core/content/login.html"];
...
-(NSURLSessionTask*) postCqActionsWithAuthorizableId: (NSString*) authorizableId
    ...
    if (authorizableId == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kOAICqApiErrorDomain code:kOAICqApiMissingParamErrorCode userInfo:userInfo];
            handler(error);
        }
        return nil;
    }
    ...
    if (changelog == nil) {
        NSParameterAssert(changelog);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"changelog"] };
            NSError* error = [NSError errorWithDomain:kOAICqApiErrorDomain code:kOAICqApiMissingParamErrorCode userInfo:userInfo];
            handler(error);
        }
        return nil;
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/.cqactions.html"];
