...
(defn-spec post-tree-activation-with-http-info any?
  ""
  [ignoredeactivated boolean?, onlymodified boolean?, path string?, cmd string?]
  (check-required-params ignoredeactivated onlymodified path cmd)
  (call-api "/libs/replication/treeactivation.html" :post
            {:path-params   {}
             :header-params {}
             :query-params  {"ignoredeactivated" ignoredeactivated "onlymodified" onlymodified "path" path "cmd" cmd }
             :form-params   {}
             :content-types []
             :accepts       []
             :auth-names    ["aemAuth"]}))
...
(defn-spec post-tree-activation any?
  ""
  [ignoredeactivated boolean?, onlymodified boolean?, path string?, cmd string?]
  (let [res (:data (post-tree-activation-with-http-info ignoredeactivated onlymodified path cmd))]
    (if (:decode-models *api-context*)
       (st/decode any? res st/string-transformer)
       res)))
