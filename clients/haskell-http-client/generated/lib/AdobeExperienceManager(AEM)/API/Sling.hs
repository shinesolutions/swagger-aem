...
instance HasOptionalParam PostAgent JcrContentAliasUpdate where
  applyOptionalParam req (JcrContentAliasUpdate xs) =
    req `addQuery` toQuery ("jcr:content/aliasUpdate", Just xs)
...
postTreeActivation (Ignoredeactivated ignoredeactivated) (Onlymodified onlymodified) (Path path) (Cmd cmd) =
  _mkRequest "POST" ["/libs/replication/treeactivation.html"]
    `_hasAuthType` (P.Proxy :: P.Proxy AuthBasicAemAuth)
