...
module AdobeExperienceManager(AEM).Core where
...
-- * AdobeExperienceManager(AEM)Config

-- | 
data AdobeExperienceManager(AEM)Config = AdobeExperienceManager(AEM)Config
...
-- | display the config
instance P.Show AdobeExperienceManager(AEM)Config where
  show c =
    T.printf
...
-- | constructs a default AdobeExperienceManager(AEM)Config
--
-- configHost:
--
-- @http://localhost@
...
-- configUserAgent:
--
-- @"adobe-experience-manager-(aem)/0.1.0.0"@
--
newConfig :: IO AdobeExperienceManager(AEM)Config
newConfig = do
    logCxt <- initLogContext
    return $ AdobeExperienceManager(AEM)Config
        { configHost = "http://localhost"
        , configUserAgent = "adobe-experience-manager-(aem)/0.1.0.0"
...
addAuthMethod :: AuthMethod auth => AdobeExperienceManager(AEM)Config -> auth -> AdobeExperienceManager(AEM)Config
addAuthMethod config@AdobeExperienceManager(AEM)Config {configAuthMethods = as} a =
  config { configAuthMethods = AnyAuthMethod a : as}

-- | updates the config to use stdout logging
withStdoutLogging :: AdobeExperienceManager(AEM)Config -> IO AdobeExperienceManager(AEM)Config
...
withStderrLogging :: AdobeExperienceManager(AEM)Config -> IO AdobeExperienceManager(AEM)Config
...
withNoLogging :: AdobeExperienceManager(AEM)Config -> AdobeExperienceManager(AEM)Config
withNoLogging p = p { configLogExecWithContext =  runNullLogExec}
 
-- * AdobeExperienceManager(AEM)Request
...
data AdobeExperienceManager(AEM)Request req contentType res accept = AdobeExperienceManager(AEM)Request
  { rMethod  :: NH.Method   -- ^ Method of AdobeExperienceManager(AEM)Request
  , rUrlPath :: [BCL.ByteString] -- ^ Endpoint of AdobeExperienceManager(AEM)Request
  , rParams   :: Params -- ^ params of AdobeExperienceManager(AEM)Request
  , rAuthTypes :: [P.TypeRep] -- ^ types of auth methods
  }
  deriving (P.Show)

-- | 'rMethod' Lens
rMethodL :: Lens_' (AdobeExperienceManager(AEM)Request req contentType res accept) NH.Method
rMethodL f AdobeExperienceManager(AEM)Request{..} = (\rMethod -> AdobeExperienceManager(AEM)Request { rMethod, ..} ) <$> f rMethod
{-# INLINE rMethodL #-}

-- | 'rUrlPath' Lens
rUrlPathL :: Lens_' (AdobeExperienceManager(AEM)Request req contentType res accept) [BCL.ByteString]
rUrlPathL f AdobeExperienceManager(AEM)Request{..} = (\rUrlPath -> AdobeExperienceManager(AEM)Request { rUrlPath, ..} ) <$> f rUrlPath
{-# INLINE rUrlPathL #-}

-- | 'rParams' Lens
rParamsL :: Lens_' (AdobeExperienceManager(AEM)Request req contentType res accept) Params
rParamsL f AdobeExperienceManager(AEM)Request{..} = (\rParams -> AdobeExperienceManager(AEM)Request { rParams, ..} ) <$> f rParams
{-# INLINE rParamsL #-}

-- | 'rParams' Lens
rAuthTypesL :: Lens_' (AdobeExperienceManager(AEM)Request req contentType res accept) [P.TypeRep]
rAuthTypesL f AdobeExperienceManager(AEM)Request{..} = (\rAuthTypes -> AdobeExperienceManager(AEM)Request { rAuthTypes, ..} ) <$> f rAuthTypes
{-# INLINE rAuthTypesL #-}
...
class HasBodyParam req param where
  setBodyParam :: forall contentType res accept. (Consumes req contentType, MimeRender contentType param) => AdobeExperienceManager(AEM)Request req contentType res accept -> param -> AdobeExperienceManager(AEM)Request req contentType res accept
  setBodyParam req xs =
...
class HasOptionalParam req param where
  ...
  applyOptionalParam :: AdobeExperienceManager(AEM)Request req contentType res accept -> param -> AdobeExperienceManager(AEM)Request req contentType res accept
  applyOptionalParam = (-&-)
  {-# INLINE applyOptionalParam #-}

  -- | infix operator \/ alias for 'addOptionalParam'
  (-&-) :: AdobeExperienceManager(AEM)Request req contentType res accept -> param -> AdobeExperienceManager(AEM)Request req contentType res accept
  (-&-) = applyOptionalParam
  {-# INLINE (-&-) #-}
...
-- ** AdobeExperienceManager(AEM)Request Utils

_mkRequest :: NH.Method -- ^ Method 
          -> [BCL.ByteString] -- ^ Endpoint
          -> AdobeExperienceManager(AEM)Request req contentType res accept -- ^ req: Request Type, res: Response Type
_mkRequest m u = AdobeExperienceManager(AEM)Request m u _mkParams []

_mkParams :: Params
_mkParams = Params [] [] ParamBodyNone

setHeader :: AdobeExperienceManager(AEM)Request req contentType res accept -> [NH.Header] -> AdobeExperienceManager(AEM)Request req contentType res accept
setHeader req header =
  req `removeHeader` P.fmap P.fst header &
  L.over (rParamsL . paramsHeadersL) (header P.++)

removeHeader :: AdobeExperienceManager(AEM)Request req contentType res accept -> [NH.HeaderName] -> AdobeExperienceManager(AEM)Request req contentType res accept
...
_setContentTypeHeader :: forall req contentType res accept. MimeType contentType => AdobeExperienceManager(AEM)Request req contentType res accept -> AdobeExperienceManager(AEM)Request req contentType res accept
_setContentTypeHeader req =
    case mimeType (P.Proxy :: P.Proxy contentType) of 
        Just m -> req `setHeader` [("content-type", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["content-type"]

_setAcceptHeader :: forall req contentType res accept. MimeType accept => AdobeExperienceManager(AEM)Request req contentType res accept -> AdobeExperienceManager(AEM)Request req contentType res accept
_setAcceptHeader req =
    case mimeType (P.Proxy :: P.Proxy accept) of 
        Just m -> req `setHeader` [("accept", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["accept"]

setQuery :: AdobeExperienceManager(AEM)Request req contentType res accept -> [NH.QueryItem] -> AdobeExperienceManager(AEM)Request req contentType res accept
setQuery req query = 
  req &
  L.over
...
addForm :: AdobeExperienceManager(AEM)Request req contentType res accept -> WH.Form -> AdobeExperienceManager(AEM)Request req contentType res accept
addForm req newform = 
...
_addMultiFormPart :: AdobeExperienceManager(AEM)Request req contentType res accept -> NH.Part -> AdobeExperienceManager(AEM)Request req contentType res accept
...
_setBodyBS :: AdobeExperienceManager(AEM)Request req contentType res accept -> B.ByteString -> AdobeExperienceManager(AEM)Request req contentType res accept
_setBodyBS req body = 
    req & L.set (rParamsL . paramsBodyL) (ParamBodyB body)

_setBodyLBS :: AdobeExperienceManager(AEM)Request req contentType res accept -> BL.ByteString -> AdobeExperienceManager(AEM)Request req contentType res accept
_setBodyLBS req body = 
    req & L.set (rParamsL . paramsBodyL) (ParamBodyBL body)

_hasAuthType :: AuthMethod authMethod => AdobeExperienceManager(AEM)Request req contentType res accept -> P.Proxy authMethod -> AdobeExperienceManager(AEM)Request req contentType res accept
...
class P.Typeable a =>
  ...
  applyAuthMethod
    :: AdobeExperienceManager(AEM)Config
    -> a
    -> AdobeExperienceManager(AEM)Request req contentType res accept
    -> IO (AdobeExperienceManager(AEM)Request req contentType res accept)
...
_applyAuthMethods
  ...
  -> AdobeExperienceManager(AEM)Config
  -> IO (AdobeExperienceManager(AEM)Request req contentType res accept)
_applyAuthMethods req config@(AdobeExperienceManager(AEM)Config {configAuthMethods = as}) =
  foldlM go req as
  where
