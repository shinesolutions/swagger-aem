...
module AdobeExperienceManager(AEM).Client where
...
dispatchLbs
  ...
  -> AdobeExperienceManager(AEM)Config -- ^ config
  -> AdobeExperienceManager(AEM)Request req contentType res accept -- ^ request
...
runConfigLog
  :: P.MonadIO m
  => AdobeExperienceManager(AEM)Config -> LogExec m a
runConfigLog config = configLogExecWithContext config (configLogContext config)

-- | Run a block using the configured logger instance (logs exceptions)
runConfigLogWithExceptions
  :: (E.MonadCatch m, P.MonadIO m)
  => T.Text -> AdobeExperienceManager(AEM)Config -> LogExec m a
