...
function Invoke-SslSetup {
    ...
    Process {
        ...
        if ($PrivatekeyFile) {
            $LocalVarFormParameters['privatekeyFile'] = $PrivatekeyFile | Foreach-Object { [System.IO.FileInfo]$executionContext.SessionState.Path.GetUnresolvedProviderPathFromPSPath($_) }
        }

        if ($CertificateFile) {
            $LocalVarFormParameters['certificateFile'] = $CertificateFile | Foreach-Object { [System.IO.FileInfo]$executionContext.SessionState.Path.GetUnresolvedProviderPathFromPSPath($_) }
        }
