...
# @param boolean $jcr:content/alias_update  (optional)
...
sub post_agent {
    ...
    # query params
    if ( exists $args{'jcr:content/alias_update'}) {
        $query_params->{'jcr:content/aliasUpdate'} = $self->{api_client}->to_query_value($args{'jcr:content/alias_update'});
    }
...
sub post_tree_activation {
    ...
    unless (exists $args{'cmd'}) {
      croak("Missing the required parameter 'cmd' when calling post_tree_activation");
    }

    # parse inputs
    my $_resource_path = '/libs/replication/treeactivation.html';
