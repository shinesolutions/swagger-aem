...
-export([delete_agent/3, delete_agent/4,
         ...
         post_tree_activation/5, post_tree_activation/6,
...
delete_agent(Ctx, Runmode, Name, Optional) ->
    ...
    Method = delete,
    Path = [?BASE_URL, "/etc/replication/agents.", Runmode, "/", Name, ""],
    QS = [],
    Headers = [],
    Body1 = [],
...
get_agents(Ctx, Runmode, Optional) ->
    ...
    Method = get,
    Path = [?BASE_URL, "/etc/replication/agents.", Runmode, ".-1.json"],
    QS = [],
    Headers = [],
    Body1 = [],
    ContentTypeHeader = openapi_utils:select_header_content_type([]),
...
get_authorizable_keystore(Ctx, IntermediatePath, AuthorizableId, Optional) ->
    ...
    Method = get,
    Path = [?BASE_URL, "/", IntermediatePath, "/", AuthorizableId, ".ks.json"],
    QS = [],
    Headers = [],
    Body1 = [],
    ContentTypeHeader = openapi_utils:select_header_content_type([]),
...
get_keystore(Ctx, IntermediatePath, AuthorizableId, Optional) ->
    ...
    Method = get,
    Path = [?BASE_URL, "/", IntermediatePath, "/", AuthorizableId, "/keystore/store.p12"],
    QS = [],
    Headers = [],
    Body1 = [],
    ContentTypeHeader = openapi_utils:select_header_content_type([]),
...
get_package(Ctx, Group, Name, Version, Optional) ->
    ...
    Method = get,
    Path = [?BASE_URL, "/etc/packages/", Group, "/", Name, "-", Version, ".zip"],
    QS = [],
    Headers = [],
    Body1 = [],
    ContentTypeHeader = openapi_utils:select_header_content_type([]),
...
get_package_filter(Ctx, Group, Name, Version, Optional) ->
    ...
    Method = get,
    Path = [?BASE_URL, "/etc/packages/", Group, "/", Name, "-", Version, ".zip/jcr:content/vlt:definition/filter.tidy.2.json"],
    QS = [],
    Headers = [],
    Body1 = [],
    ContentTypeHeader = openapi_utils:select_header_content_type([]),
...
get_query(Ctx, Path, PLimit, 1Property, 1PropertyValue, Optional) ->
    ...
    Method = get,
    Path = [?BASE_URL, "/bin/querybuilder.json"],
...
get_truststore(Ctx, Optional) ->
    ...
    Method = get,
    Path = [?BASE_URL, "/etc/truststore/truststore.p12"],
    QS = [],
    Headers = [],
    Body1 = [],
    ContentTypeHeader = openapi_utils:select_header_content_type([]),
...
get_truststore_info(Ctx, Optional) ->
    ...
    Method = get,
    Path = [?BASE_URL, "/libs/granite/security/truststore.json"],
    QS = [],
    Headers = [],
    Body1 = [],
    ContentTypeHeader = openapi_utils:select_header_content_type([]),
...
post_authorizable_keystore(Ctx, IntermediatePath, AuthorizableId, Optional) ->
    ...
    Method = post,
    Path = [?BASE_URL, "/", IntermediatePath, "/", AuthorizableId, ".ks.html"],
...
post_authorizables(Ctx, AuthorizableId, IntermediatePath, Optional) ->
    ...
    Method = post,
    Path = [?BASE_URL, "/libs/granite/security/post/authorizables"],
...
post_config_adobe_granite_saml_authentication_handler(Ctx, Optional) ->
    ...
    Method = post,
    Path = [?BASE_URL, "/apps/system/config/com.adobe.granite.auth.saml.SamlAuthenticationHandler.config"],
...
post_config_apache_felix_jetty_based_http_service(Ctx, Optional) ->
    ...
    Method = post,
    Path = [?BASE_URL, "/apps/system/config/org.apache.felix.http"],
...
post_config_apache_http_components_proxy_configuration(Ctx, Optional) ->
    ...
    Method = post,
    Path = [?BASE_URL, "/apps/system/config/org.apache.http.proxyconfigurator.config"],
...
post_config_apache_sling_dav_ex_servlet(Ctx, Optional) ->
    ...
    Method = post,
    Path = [?BASE_URL, "/apps/system/config/org.apache.sling.jcr.davex.impl.servlets.SlingDavExServlet"],
...
post_config_apache_sling_get_servlet(Ctx, Optional) ->
    ...
    Method = post,
    Path = [?BASE_URL, "/apps/system/config/org.apache.sling.servlets.get.DefaultGetServlet"],
...
post_config_apache_sling_referrer_filter(Ctx, Optional) ->
    ...
    Method = post,
    Path = [?BASE_URL, "/apps/system/config/org.apache.sling.security.impl.ReferrerFilter"],
...
post_config_property(Ctx, ConfigNodeName, Optional) ->
    ...
    Method = post,
    Path = [?BASE_URL, "/apps/system/config/", ConfigNodeName, ""],
    QS = [],
    Headers = [],
    Body1 = [],
    ContentTypeHeader = openapi_utils:select_header_content_type([]),
...
post_node_rw(Ctx, Path, Name, Optional) ->
    ...
    Method = post,
    Path = [?BASE_URL, "/", Path, "/", Name, ".rw.html"],
...
%% @doc 
%% 
-spec post_tree_activation(ctx:ctx(), boolean(), boolean(), binary(), binary()) -> {ok, [], openapi_utils:response_info()} | {ok, hackney:client_ref()} | {error, term(), openapi_utils:response_info()}.
post_tree_activation(Ctx, Ignoredeactivated, Onlymodified, Path, Cmd) ->
    post_tree_activation(Ctx, Ignoredeactivated, Onlymodified, Path, Cmd, #{}).

-spec post_tree_activation(ctx:ctx(), boolean(), boolean(), binary(), binary(), maps:map()) -> {ok, [], openapi_utils:response_info()} | {ok, hackney:client_ref()} | {error, term(), openapi_utils:response_info()}.
post_tree_activation(Ctx, Ignoredeactivated, Onlymodified, Path, Cmd, Optional) ->
    _OptionalParams = maps:get(params, Optional, #{}),
    Cfg = maps:get(cfg, Optional, application:get_env(openapi_api, config, #{})),

    Method = post,
    Path = [?BASE_URL, "/libs/replication/treeactivation.html"],
    QS = lists:flatten([{<<"ignoredeactivated">>, Ignoredeactivated}, {<<"onlymodified">>, Onlymodified}, {<<"path">>, Path}, {<<"cmd">>, Cmd}])++openapi_utils:optional_params([], _OptionalParams),
    Headers = [],
    Body1 = [],
    ContentTypeHeader = openapi_utils:select_header_content_type([]),
    Opts = maps:get(hackney_opts, Optional, []),
...
post_truststore(Ctx, Optional) ->
    ...
    Method = post,
    Path = [?BASE_URL, "/libs/granite/security/post/truststore"],
