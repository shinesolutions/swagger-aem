...
nlohmann::json TruststoreInfo::toJson() const
{
...
void TruststoreInfo::fromJson(nlohmann::json& val)
{
    {
        m_Aliases.clear();
        nlohmann::json jsonArray;
        ...
        for( auto& item : val["aliases"] )
        {
            ...
            if(item.is_null())
            {
                m_Aliases.push_back( TruststoreItems() );
            }
            else
            {
                TruststoreItems newItem;
...
std::vector<TruststoreItems>& TruststoreInfo::getAliases()
{
    return m_Aliases;
}
