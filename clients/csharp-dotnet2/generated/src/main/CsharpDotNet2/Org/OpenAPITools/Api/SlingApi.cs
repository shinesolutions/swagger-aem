...
if (keyStore != null) fileParams.Add("keyStore", ApiClient.ParameterToFile("keyStore", keyStore));
 ...
 if (filterMethodsTypeHint != null) queryParams.Add("filter.methods@TypeHint", ApiClient.ParameterToString(filterMethodsTypeHint)); // query parameter
        ...
        public void PostConfigProperty (string configNodeName)
        {
            ...
            if (configNodeName == null) throw new ApiException(400, "Missing required parameter 'configNodeName' when calling PostConfigProperty");
            

            var path = "/apps/system/config/{configNodeName}";
            path = path.Replace("{format}", "json");
            path = path.Replace("{" + "configNodeName" + "}", ApiClient.ParameterToString(configNodeName));
            ...
            if (((int)response.StatusCode) >= 400)
                throw new ApiException ((int)response.StatusCode, "Error calling PostConfigProperty: " + response.Content, response.Content);
            else if (((int)response.StatusCode) == 0)
                throw new ApiException ((int)response.StatusCode, "Error calling PostConfigProperty: " + response.ErrorMessage, response.ErrorMessage);
